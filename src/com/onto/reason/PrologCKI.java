...
public class PrologCKI {
    ...
    public void reasonTest(){
         ...
         consultFile("e:/excel/peopleold.pl");
         Vector v = solve("r2100000021(1100001312,1100001312).");
         for (int i=0; i<v.size(); i++) {
              System.out.println("\nNext answer:");
              Hashtable the_answers = (Hashtable)v.elementAt(i);
              Enumeration enumPl = the_answers.keys();
              while (enumPl.hasMoreElements()) {
               String var = (String)enumPl.nextElement();
...
public ArrayList<ArrayList<String>> reasonPro(String pathandname,String rule){
     ArrayList<ArrayList<String>> results=new ArrayList<ArrayList<String>>();
         consultFile(pathandname);
         Vector v = solve(rule);
        ...
        if((v.size()==0)&&(hasSolution!="yes")) temp.add("no");
         ...
         for (int i=0; i<v.size(); i++) {
          //    System.out.println("\nNext answer:");
              ...
              while (enumPl.hasMoreElements()) {
               String var = (String)enumPl.nextElement();
               String val = (String)the_answers.get(var);
               if(resu==0) temp0.add(val);
               if(resu==1) temp1.add(val);
               resu=resu+1;
    ...
    public void assertPl(String s) {
        term inp=(new prologtokenizer(s)).gettermdot(null);
        if (sv.assertPl(inp) == false) {
            System.out.println("Error assertPling: " + s);
        }
    }
    public Vector solve(String s) {
        term inp=(new prologtokenizer(s)).gettermdot(null);
        hasSolution=sv.query(inp);
    ...
    static public void main(String [] args) {
         ...
         p.reasonPro("e:/excel/people.pl","r2100000001(1000000035,1000000004).");
      //   p.reasonTest();
...
//          */
//         PrologCKI p = new PrologCKI();
//         p.assertPl("father(ken,mark).");
//         p.assertPl("father(ken,ron).");
//         p.assertPl("father(ron,anthony).");
//         p.assertPl("grandfather(X,Z):-father(X,Y),father(Y,Z).");
...
class program {
    ...
    static void makeprelude() {
        ...
        assertPl(prelude,"member(X,[X|_]).");
        assertPl(prelude,"member(X,[_|H]):-member(X,H).");
        assertPl(prelude,"not(X):-X,!,fail.");
        assertPl(prelude,"not(X).");
        assertPl(prelude,"append([],B,B).");
        assertPl(prelude,"append([A|B],C,[A|D]):-append(B,C,D).");
        assertPl(prelude,"select([X|B],X,B).");
        assertPl(prelude,"select([A|B],X,[A|C]):-select(B,X,C).");
        assertPl(prelude,"reverse(X,XR):-reverse(X,[],XR).");
        assertPl(prelude,"reverse([],XR,XR).");
        assertPl(prelude,"reverse([H|T],TR,XR):-reverse(T,[H|TR],XR).");
        assertPl(prelude,"permutation([],[]).");
        assertPl(prelude,"permutation(LX,[X|LP]):-select(LX,X,L),permutation(L,LP).");
    }
    ...
    static void fillwith(Hashtable to,Hashtable from) {
        Enumeration enumPl=from.elements();
        term oldlist,newlist;
        while(enumPl.hasMoreElements()) {
            oldlist=(term)enumPl.nextElement();
    ...
    public String toString() {
        Enumeration enumPl=user.elements();
        Vector v;
        term t;
        StringBuffer buf=new StringBuffer("\n");
        while(enumPl.hasMoreElements()) {
            term list=(term)enumPl.nextElement();
    ...
    static boolean assertPl(Hashtable h,String s)
    {return assertPl(h,new prologtokenizer(s).gettermdot(null));}
    ...
    static boolean assertPl(Hashtable h,term t) {
    ...
    static boolean assertPla(Hashtable h,term t) {
        if(t==null)  return false;
        //t=term.skipeq(t);
        term thead=head(t);
        if(thead==null)  return false;
...
class solver {
    ...
    int solve(rack r) {
        ...
        if(bi!=-1) {
            ...
            case 26:
                r.solveoption=r.NOTAGAIN;
                if(assertPl(program.gramconvert(rpred.arg[0].copy())
                          ))
                    return TRUE;
                return ERROR;
            case 27:
                r.solveoption=r.NOTAGAIN;
                if(program.assertPla(lib.user,
    ...
    public String run(int max_iter) {
        rack current;
        wait=false;
        int substnum;
    ...
    boolean assertPl(term X)
    {  return program.assertPl(lib.user,X);
    }
